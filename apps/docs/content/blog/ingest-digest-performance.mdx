...
- The digest future is boxed only after it holds a permit. Waiters are now a few bytes each, so backlog memory is bounded by the concurrency cap instead of the burst size.
...
PostgreSQL gets everything that is not SQLite-specific: bounded backlog memory, no replay of the live queue, one blocking round trip per stored event, and five fewer statements per digest.

We also tried `mimalloc` and did not adopt it. The allocator is 3% of the profile and memory is now bounded by the backlog fix, so it did not justify a C build dependency in the distroless image.
